...
```{r f-test-linear-restrictions, eval = FALSE}
df %>% 
  lm(y ~ x1 + x2 + x3, .) %>% 
...
```{r f-test-cobb-douglas, eval=FALSE}
df %>% 
...
```{r unrestricted_model, comment=NA, warning=F, message=F}
mtcars %>% 
  lm(mpg ~ cyl + disp + hp, .) %>% 
  jtools::summ()
```
