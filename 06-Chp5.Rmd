...
Let's test the linear restriction for the Cobb-Douglas model above.  Estimate the model as `ols_u <- df %>% lm(log(q) ~ log(l) + log(k), .)`^[There can be no values `<= 0`.] To test the hypothesis $\alpha=\beta$ pipe ols_u into `linearHypothesis` with the argument $c(log(l) = log(k))$: `ols_u %>% linearHypothesis(c("log(l) = log(k)"))`.  Again, the *F*-statistic, its degrees of freedom, and *p-value* will be returned.
...
```{r wald-test-tidy, message=FALSE}
# estimate the unrestricted model and retrieve ssr
unrestricted <- 
mtcars %>% 
...
# estimate the restricted model and retrieve ssr
restricted <- 
mtcars %>% 
...
bind_cols(unrestricted, restricted) %>% 
  mutate(numerator = (deviance...3 - deviance...1) / (df.residual...4 - df.residual...2),
         denominator = deviance...1 / df.residual...2,
         F_calc = numerator / denominator,
         F_crit = qf(.95, df.residual...4 - df.residual...2, df.residual...2),
         p_value = pf(F_calc, df.residual...4 - df.residual...2, df.residual...2, 
                      lower.tail = FALSE)) %>% 
  select(F_calc, F_crit, p_value) ->
  wald

wald
```

Since `r wald$F_calc` is greater than `r wald$F_crit` we can reject $H_0$ in favor of $H_1$ and conclude that *wt*, *am*, and *carb* add significant explanatory power to the model.  We can also see that the p-value for our calculated *F*-statistic is `r pf(F_stat, q, df_u, lower.tail = F)`.  Since this is less than $\alpha=.05$ we reject $H_0$.
